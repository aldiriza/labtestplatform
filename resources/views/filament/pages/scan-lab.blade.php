        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="p-6 bg-white rounded-xl shadow-sm border border-gray-200">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-bold tracking-tight text-gray-900">Scan Lab QR Code</h2>
                    <span
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ $scannedMaterial->material_name }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">Unique ID</p>
                                <p class="font-mono text-gray-700">{{ $scannedMaterial->unique_id }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">Supplier</p>
                                <p class="text-gray-900">{{ $scannedMaterial->supplier }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">PO Number</p>
                                <p class="text-gray-900">{{ $scannedMaterial->po_number }}</p>
                            </div>
                        </div>

                        {{-- Result Entry Form --}}
                        @if($scannedMaterial->status === 'lab_in_progress' && (auth()->user()->hasRole('lab') || auth()->user()->hasRole('admin')))
                            <div class="mt-6 pt-6 border-t border-gray-100 bg-gray-50 -mx-6 px-6 pb-2">
                                <h4 class="font-semibold text-gray-900 mb-4 flex items-center">
                                    <x-heroicon-o-clipboard-document-check class="w-5 h-5 mr-2 text-gray-500" />
                                    Enter Test Result
                                </h4>
                                <form wire:submit="submitResult">
                                    {{ $this->form->schema($this->getResultFormSchema())->statePath('resultData') }}
                ...
                @if(count($recentScans) > 0)
                    <div class="space-y-3">
                        @foreach($recentScans as $scan)
                            <div class="flex items-center space-x-3 p-3 rounded-lg bg-gray-50 border border-gray-100">
